       ...
       ENTRY 'GET-RANDOM-CONSUMABLE' USING OUTPUT-STRING.
           ...
           CALL 'get-random-int' USING 
               BY REFERENCE TABLE-IDX.
      *             BY VALUE CORE-CONSUMABLES-LENGTH.
      * TODO: not working with passing length as a parameter

           MOVE CORE-CONSUMABLE(TABLE-IDX) 
               TO FOUND-CONSUMABLE(TABLE-IDX).

           IF CONSUMABLE-COUNT OF FOUND-CONSUMABLE(TABLE-IDX) > 1 THEN
               STRING 
                   FUNCTION 
                   TRIM(CONSUMABLE-NAME OF FOUND-CONSUMABLE(TABLE-IDX))
                   ' ('
                   CONSUMABLE-COUNT OF FOUND-CONSUMABLE(TABLE-IDX)
                   ')'
                   INTO OUTPUT-STRING
           ELSE
               MOVE CONSUMABLE-NAME OF FOUND-CONSUMABLE(TABLE-IDX) 
               TO OUTPUT-STRING
           END-IF.
